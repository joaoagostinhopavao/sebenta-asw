...
### Listagem de Países (`index.html`)

Uma tabela *Bootstrap*, com alguns países fixos no HTML, a título de exemplo. Nos LABs, esta tabela passa a ser preenchida dinamicamente a partir do resultado de `/country`.
...
Um formulário com os campos `user` e `pass`, com os mesmos nomes que o *endpoint* `/login` espera receber:
...
### Consulta de Capital (`capital.html`)

Um formulário simples, com um único campo `countrycode`, alinhado com o exemplo já apresentado no [Capítulo 1](cap01.qmd):
...
### Gestão de Países (`paises-editar.html`)

Três formulários distintos, um por operação, todos a apontar para o mesmo *endpoint* `/countryEdit`, distinguidos pelo campo escondido `operation` (ver a nota sobre este desenho no [Capítulo 6](cap06.qmd)):
