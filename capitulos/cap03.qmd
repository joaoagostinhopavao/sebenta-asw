...
## A Plataforma Jakarta EE {#sec-03-jakarta-ee}
...
## Arquitetura em Camadas {#sec-03-arquitetura-camadas}
...
![A arquitetura em camadas da Jakarta EE: o cliente comunica apenas com a Web Tier, que por sua vez recorre à Business Tier e, através dela, à base de dados na EIS Tier.](../assets/images/capitulos/cap03/Arquitetura_JAVA_EE.png){#fig-03-arquitetura-jakarta-ee width="70%"}
...
## Porquê Servlets? {#sec-03-porque-servlets}
...
![Do ponto de vista do cliente, uma Servlet é apenas mais um destino de pedidos HTTP: recebe um pedido, devolve uma resposta.](../assets/images/capitulos/cap03/Servlet_HTTP.png){#fig-03-servlet-http width="70%"}

## O Ciclo de Vida de uma Servlet {#sec-03-ciclo-vida}
...
![O contentor carrega a classe da Servlet uma única vez; cada pedido recebido é despachado para um thread próprio, que invoca `service()` sobre essa mesma instância partilhada.](../assets/images/capitulos/cap03/Servlet_live_cycle.png){#fig-03-servlet-ciclo-vida width="80%"}

## Da Mensagem HTTP ao Método Java {#sec-03-mapeamento-http-servlet}
...
Um pedido a `http://localhost:8080/country` é assim encaminhado para a classe `CountryServlet`. Dentro dela, os elementos da mensagem HTTP (ver [Capítulo 1](cap01.qmd)) correspondem a métodos concretos dos objetos `request` e `response`:
...
![Dentro do servidor Jakarta EE, o pedido chega encapsulado num `HttpServletRequest`, é processado pelo componente Web (a Servlet), que pode consultar recursos de dados, e a resposta sai encapsulada num `HttpServletResponse`.](../assets/images/capitulos/cap03/HTTP_Request_server_3.png){#fig-03-http-request-server width="80%"}

## Content-Type e a Forma da Resposta {#sec-03-content-type}
...
Para definir o código de estado da resposta (ver [Capítulo 1](cap01.qmd)), a classe `HttpServletResponse` disponibiliza uma constante para cada código, evitando o uso de números "mágicos" diretamente no código:
...
## Uma Primeira Servlet: "World Capitals" {#sec-03-primeiro-servlet}
...
## Servlets e o Padrão MVC: Falta a View {#sec-03-servlets-mvc}
...
![Na realização clássica do MVC em Jakarta EE, a Servlet funciona como Controller, uma JavaServer Page como View, e uma classe Java simples como Model.](../assets/images/capitulos/cap03/Arquitetura_MVC.png){#fig-03-mvc-servlet-jsp width="70%"}

Essa página dedicada, a *JavaServer Page* (JSP), é abordada em detalhe no [Capítulo 4](cap04.qmd).
