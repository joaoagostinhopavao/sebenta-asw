...
## O Problema: Segurança Repetida em Cada *Servlet* {#sec-06-problema}

Com o que foi apresentado até aqui (ver [Capítulo 5](cap05.qmd)), a autenticação está resolvida: a `LoginServlet` valida as credenciais e grava o utilizador na sessão, e cada *endpoint* protegido verifica essa sessão no início do seu `doGet()` ou `doPost()`. Isto funciona, mas tem um custo escondido: a lógica de autenticação vive, duplicada, dentro de **cada** *Servlet* protegida.
...
## Autenticação vs. Autorização {#sec-06-autenticacao-autorizacao}
...
A autenticação acontece sempre primeiro: não faz sentido verificar permissões de alguém cuja identidade ainda não foi confirmada. Esta distinção reflete-se diretamente nos códigos de estado HTTP usados na resposta (ver [Capítulo 3](cap03.qmd)):
...
## O que é um Filtro {#sec-06-o-que-e-um-filter}
...
Um [Filtro](https://jakarta.ee/specifications/servlet/) é um componente do lado do servidor que intercepta pedidos HTTP:
...
## Onde um Filtro se Situa no Pedido {#sec-06-onde-situa}
...
![Um pedido atravessa a cadeia de Filtros antes de chegar à Servlet; cada Filtro pode bloquear o pedido (vermelho), modificá-lo (amarelo) ou deixá-lo prosseguir (verde, chamando chain.doFilter()).](../assets/images/capitulos/cap06/Filter_Chain.png){#fig-06-filter-chain width="80%"}
...
## Ciclo de Vida de um Filtro {#sec-06-ciclo-vida-filter}

O ciclo de vida de um Filtro segue o mesmo modelo já apresentado para as *Servlets* (ver [Capítulo 4](cap04.qmd)):
...
![O ciclo de vida de um Filtro espelha o de uma Servlet: init() e destroy() correm uma única vez, doFilter() corre a cada pedido, para múltiplos pedidos concorrentes.](../assets/images/capitulos/cap06/Filter_Lifecycle.png){#fig-06-filter-lifecycle width="85%"}

## *Servlet* e Filtro: Separação de Responsabilidades {#sec-06-separacao-responsabilidades}

Centralizar a segurança num Filtro reorganiza as responsabilidades de toda a aplicação:

![Sem Filtro, cada Servlet acumula autenticação, logging, validação e lógica de negócio. Com um Filtro, essas preocupações transversais concentram-se num único sítio, e cada Servlet fica só com a sua lógica de negócio.](../assets/images/capitulos/cap06/Servlet_vs_Filter.png){#fig-06-servlet-vs-filter width="75%"}
...
## Declarar e Implementar um Filtro {#sec-06-declarar-filter}
...
## Redirecionamento vs. `401`: uma Diferença Arquitetural {#sec-06-redirect-vs-401}
...
- Numa API REST, cujo contrato é JSON (ver [Capítulos 7](cap07.qmd) e [8](cap08.qmd)), a resposta correta é devolver **`401`**, sem qualquer redirecionamento
